...
function RecordingContainer({ userEmail, accessToken }) {
    ...
    const handleStartRecording = async () => {
        try {
            ...
            let recordingFormattedDate = recordingStartDate || formatTimeStamp(new Date());
            ...
            const socket = new WebSocket('wss://api.deepgram.com/v1/listen?model=nova-2-medical&smart_format=true', [
                'token',
                '3302291b9d64b7db3a6310139b50b3e9e60dc6bb',
            ]);
            ...
            socket.onmessage = (message) => {
                ...
                if (transcript && data.is_final) {
                    ...
                    let transcriptSavedObject = JSON.parse(localStorage.getItem("transcript")) || {};
                    if (!transcriptSavedObject[email]) {
                        transcriptSavedObject[email] = [];
                    }

                    const existingIndex = transcriptSavedObject[email].findIndex((item) => item.id === transcriptObject.id);
                    if (existingIndex !== -1) {
                            ...
                            {!recording ? (
                                ...
                                <>
                                    ...
                                    <div className="flex justify-center mt-10 space-x-4">
                                        ...
                                        >
                                            {isPaused ? (
                                                <Image src={playbuttonSvg} className="w-14 ml-2" alt='play button'></Image>
                                            ) : (
